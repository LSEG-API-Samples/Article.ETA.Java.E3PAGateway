...
docker run --net=host -p 14003:14003 -v ${HOST_LOG_DIRECTORY}:/var/log/APAGateway -v ${HOST_RULES_DIRECTORY}:/etc/APAGateway/rules -t ${DOCKER_IMAGE} --fixaddress ${FIXIP} --fixport ${FIXPORT} --serviceid ${SERVICEID} --sender ${SENDER} --target ${TARGET} --verbose --service ${SERVICENAME} --nofix --server
