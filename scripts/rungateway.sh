...
./gateway.sh --filelog ${FIXLOGS} --fixaddress ${FIXIP} --fixport ${FIXPORT} --serviceid ${SERVICEID} --sender ${SENDER} --target ${TARGET} --rulesfile ${HOST_RULES_FILE} --verbose --service ${SERVICENAME} --server --fixdictionary ${DICTIONARY_PATH}/FDMFixFieldDictionary --fixenums ${DICTIONARY_PATH}/FDMenumtypes.def --postdictionary ${DICTIONARY_PATH}/RDMFieldDictionary --postenums ${DICTIONARY_PATH}/enumtype.def
